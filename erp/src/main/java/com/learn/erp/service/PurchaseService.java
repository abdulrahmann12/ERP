...
public class PurchaseService {
	...
	public PurchaseResponseDTO createPurchase(Long userId, @Valid PurchaseCreateDTO dto) {
		...
		Purchase purchase = Purchase.builder().user(user).supplier(supplier).notes(dto.getNotes()).build();
		...
		for (PurchaseItemCreateDTO itemDTO : dto.getItems()) {
			...
			PurchaseItem item = PurchaseItem.builder().product(product).price(lineTotal).purchase(purchase)
		...
		TransactionSynchronizationManager.registerSynchronization(new AfterCommitExecutor() {
			...
			public void afterCommit() {
				try {
					...
					emailService.sendPurchaseInvoiceWithPdf(supplier.getEmail(), saved.getPurchaseId(),
							saved.getTotalAmount(), pdfBytes);
				} catch (Exception e) {
	...
	public BigDecimal getSupplierTotalSales(Long supplierId) {
		...
		List<Purchase> purchases = purchaseRepository.findBySupplier(supplier);
		return purchases.stream().map(Purchase::getTotalAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
	}
	...
	public SupplierPurchaseReportDTO generateSupplierPurchaseReport(Long supplierId) {
		...
		return SupplierPurchaseReportDTO.builder().supplier(supplierMapper.toDTO(supplier)).totalSales(totalAmount)
				.numberOfOrders(purchases.size()).build();
	}
