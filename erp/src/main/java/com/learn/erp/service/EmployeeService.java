...
public class EmployeeService {
	...
	public EmployeeDetailsResponseDTO addNewEmployee(@Valid EmployeeDetailsCreateRequestDTO dto) {
		User user = userRepository.findById(dto.getUserId()).orElseThrow(() -> new UserNotFoundException());
		...
		AddEmployeeEvent event = new AddEmployeeEvent(user.getEmail(), user.getUsername());
		rabbitTemplate.convertAndSend(EMPLOYEE_EXCHANGE, NEW_EMPLOYEE_KEY, event);
